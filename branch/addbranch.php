...
echo "<div class='container-fluidser' style='margin-left: 66px'><br><h1>Add branch details:</h1>";
echo "<form action='target_addbranch.php' method='POST'>
<br>
ID: <input type='number' name='ID' class='form-control' required=true> <br>
City ID: <select name='city' class='form-control' required=true><option>Please choose an option below</option>";
...
echo "</select><br>
State ID: <select name='state' class='form-control' required=true><option>Please choose an option below</option>";

$sql="SELECT ID, state from `state`";
$result = $conn->query($sql);

while ($row = $result->fetch_assoc()){
  echo "<option value='".$row['ID']."'>".$row['ID']."&nbsp;&nbsp;&nbsp;".$row['state']."</option>";

}
echo "</select><br>
Address: <input type='text' name='address' class='form-control' required=true> <br>
Pincode: <input type='number' name='pincode' class='form-control'> <br><button type='submit' class='btn btn-success form-control'>Add new staff</button>
</form></div>";
