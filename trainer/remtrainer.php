...
//
echo "<div class='container'><br><h1>Remove a trainer:</h1>";
echo "<form action='target_remtrainer.php' method='post'>
 <br><select name='ID' class='form-control' required=true><option>Please choose an option below</option>";

$sql="SELECT ID, name, specialities from `trainer`";
$result = $conn->query($sql);

while ($row = $result->fetch_assoc()){
//  echo "<option value='".$row['ID']."'> G".$row['ID']."&nbsp;&nbsp;&nbsp;".$row['type']."&nbsp;&nbsp;&nbsp;".$row['size']."mm</option>";
  echo "<option value='".$row['ID']."'>".$row['ID']."&nbsp;&nbsp;&nbsp;".$row['name']."&nbsp;&nbsp;&nbsp;".$row['specialities']."</option>";
