...
//
echo "<div class='container'><br><h1>Remove workout class:</h1>";
echo "<form action='target_remworkout.php' method='post'>
 <br><select name='ID' class='form-control' required=true><option>Please choose an option below</option>";

$sql="SELECT class_id, title from `workout`";
$result = $conn->query($sql);

while ($row = $result->fetch_assoc()){
  echo "<option value='".$row['class_id']."'>".$row['class_id']."&nbsp;&nbsp;&nbsp;".$row['title']."</option>";
